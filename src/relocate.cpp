...
void relocate(relocs_t *relocs, uint8_t *file_memory, size_t length) {
  ...
  if (relocs->symtab && relocs->strtab && relocs->strtab_size) {
    ...
    /*if (relocs->strtab_size + reinterpret_cast<uintptr_t>(relocs->strtab) -
  ...
  /*if (reinterpret_cast<uintptr_t>(relocs->rela + relocs->rela_count) -
          reinterpret_cast<uintptr_t>(file_memory) >
      length)
  ...
  for (auto rela = relocs->rela; rela < relocs->rela + relocs->rela_count;
       ++rela) {
    /*if (reinterpret_cast<uintptr_t>(rela) -
            reinterpret_cast<uintptr_t>(file_memory) >
        length)
      error("EOF\n");*/ // TODO: Make this work again
    ...
    case R_SH_RELATIVE:
      *addr = static_cast<uint32_t>(reinterpret_cast<uintptr_t>(
      ...
      break;
    case R_SH_DIR32:
      *addr = reinterpret_cast<uintptr_t>(lookup(reinterpret_cast<void *>(
